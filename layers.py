...
class Layers:
    ...
    def inference0(self, input_x, is_training=True, reuse=False):
        with tf.variable_scope("inference0") as gs:
            if reuse:
                gs.reuse_variables()
    ...
    def ladder0(self, input_x, is_training=True, reuse=False):
        with tf.variable_scope("ladder0") as gs:
            if reuse:
                gs.reuse_variables()
            conv1 = conv2d_bn_lrelu(input_x, self.network.cs[1], [4, 4], 2, is_training)
            conv2 = conv2d_bn_lrelu(conv1, self.network.cs[2], [4, 4], 2, is_training)
    ...
    def inference1(self, latent1, is_training=True, reuse=False):
        with tf.variable_scope("inference1") as gs:
            if reuse:
                gs.reuse_variables()
    ...
    def ladder1(self, latent1, is_training=True, reuse=False):
        with tf.variable_scope("ladder1") as gs:
            if reuse:
                gs.reuse_variables()
            fc1 = fc_bn_lrelu(latent1, self.network.cs[3], is_training)
            fc2 = fc_bn_lrelu(fc1, self.network.cs[3], is_training)
    ...
    def ladder2(self, latent1, is_training=True, reuse=False):
        with tf.variable_scope("ladder2") as gs:
            if reuse:
                gs.reuse_variables()
            fc1 = fc_bn_lrelu(latent1, self.network.cs[3], is_training)
            fc2 = fc_bn_lrelu(fc1, self.network.cs[3], is_training)
    ...
    def ladder2_gumbel(self, latent1, num_categories, reuse=False):
        with tf.variable_scope("ladder2") as gs:
            if reuse:
                gs.reuse_variables()
